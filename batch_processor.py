...
class BatchWorker(QThread):
    ...
    def save_simulation_result(self, index, config, result):
        ...
        try:
            ...
            # Save DF/F0 data if present
            if 'df_f0_data' in result and result['df_f0_data'] is not None:
                dff0_path = os.path.join(sim_dir, "df_f0_data.csv")
                with open(dff0_path, 'w', newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    ...
                    # Write DF/F0 data for each well
                    for i, well_data in enumerate(result['df_f0_data']):
                        row, col = i // cols, i % cols
                        well_id = f"{chr(65 + row)}{col + 1}"
                        writer.writerow([well_id] + list(well_data))
    ...
    def create_summary_plot(self, result, output_path):
        ...
        try:
            ...
            # Check if DF/F0 is enabled
            use_df_f0 = result['params'].get('simulate_df_f0', False) and 'df_f0_data' in result

            if use_df_f0:
                plot_data = result['df_f0_data']
                y_label = 'ΔF/F₀ (%)' if result['params'].get('df_f0_as_percent', True) else 'ΔF/F₀ (ratio)'
            else:
                plot_data = plate_data
            ...
            for i, well_data in enumerate(plot_data):
                ...
                for i, well_data in enumerate(plot_data):
                    ...
                    if row < 8 and col < 12:
                        if len(well_data) > baseline_end:
                            if use_df_f0:
                                # For DF/F0 data, we just need the peak value
                                heatmap_data[row, col] = np.max(well_data[baseline_end:])
                            else:
            ...
            # Set the colorbar label based on data type
            if use_df_f0:
                plt.colorbar(im, ax=ax2, label=y_label)
            else:
